...
public class Drone implements Serializable {
    ...
    private final VetoableChangeSupport droneVetoableSupport;
    ...
    public Drone(Location initLoc) {
        droneVetoableSupport = new VetoableChangeSupport(this);
        try {
            takeOff(initLoc);
     ...
     */
    private void takeOff(Location initLoc) throws PropertyVetoException {
        ...
        if(initLoc.getX() < 0 || initLoc.getX() > Configurations.SPAWN_AREA_X ||
                initLoc.getY() < 0 || initLoc.getY() > Configurations.SPAWN_AREA_Y) {
            Location defaultLoc = new Location(0, 0);
            loc = defaultLoc;
            // Firing the position, the JFrame will handle if defaultLoc is out of bounds
            droneVetoableSupport.fireVetoableChange(PROP_LOC_PROPERTY, initLoc, defaultLoc);
            GUIDrones.updateGUI();
        }
    ...
    public void land() {
        ...
        if(flying) {
            ...
            timer.cancel();
            try {
                droneVetoableSupport.fireVetoableChange(PROP_FLY_PROPERTY, true, flying);
            } catch (PropertyVetoException ex) {}
        }
    }
    ...
    /**
     * Randomly moves the drone around its actual position."(with a displacement in each direction within -10 and +10)"
     * @throws java.beans.PropertyVetoException
     */
    public void move() throws PropertyVetoException {
     ...
     */
    private void setLocProperty(double dx, double dy) throws PropertyVetoException {
        try {
            Location oldValue = loc.clone();
            loc.translate(dx, dy);
            droneVetoableSupport.fireVetoableChange(PROP_LOC_PROPERTY, oldValue, this.loc);
    ...
    public void addVetoableChangeListener(VetoableChangeListener listener) {
        droneVetoableSupport.addVetoableChangeListener(listener);
    }
    ...
    public void removeVetoableChangeListener(VetoableChangeListener listener) {
        droneVetoableSupport.removeVetoableChangeListener(listener);
    }
