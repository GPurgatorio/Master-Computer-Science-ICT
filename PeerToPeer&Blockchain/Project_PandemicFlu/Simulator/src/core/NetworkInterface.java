...
abstract public class NetworkInterface implements ModuleCommunicationListener {
	...
	private Set<DTNHost> connectedNodes = new HashSet<>();
	...
	private void notifyConnectionListeners(int type, DTNHost otherHost) {
		...
		for (ConnectionListener cl : this.cListeners) {
			switch (type) {
			case CON_UP:
				if(connectedNodes.add(otherHost))
					infect(this.host, otherHost);
				cl.hostsConnected(this.host, otherHost);
				break;
			case CON_DOWN:
				connectedNodes.remove(otherHost);
	...
	/**
	 * If given hosts share the same OS and one of the two is infected, it'll infect the other one
	 * @param host1 first host
	 * @param host2 second host
	 */
	public void infect(DTNHost host1, DTNHost host2) {

		Settings s = new Settings("PandemicFlu");
		double propagationProb = s.getDouble("propagationProb");
		if(propagationProb < 0 || propagationProb > 1) {
			System.err.println("Error in configurations. PatchProb isn't a valid value [0,1] -> " + propagationProb);
			System.exit(-1);
		}

		// Simple check for the success probability
		if(rng.nextDouble() > propagationProb)
			return;


		// If they share the same OS and one of the two is infected (XOR)
		// If they share the same OS
		if(host1.getOS().equals(host2.getOS())) {
			// and only one of them is infected
			if(host1.isInfectious() && host2.isSusceptible()) {
				host2.becomeInfected();
				System.out.println(host1.toString() + " infected " + host2.toString() + " at " + SimClock.getIntTime());
			}
			else if (host2.isInfectious() && host1.isSusceptible()) {
				host1.becomeInfected();
				System.out.println(host2.toString() + " infected " + host1.toString() + " at " + SimClock.getIntTime());
			}
		}
	}
